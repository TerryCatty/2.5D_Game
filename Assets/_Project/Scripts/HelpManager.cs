...
public class HelpManager : Localize
{
    ...
    private void CheckLanguage()
    {
        ...
        {
            LocalizeFolders folder = GameManager.instance.folders.First(folder => folder.name == this.folder);
            string json = folder.textList.First(text => text.name == list).text;
            string keyJson = helpList[i].key;
            helpList[i].text = GetLocalizeText(json, list, keyJson, GameManager.instance.language);
        }
    }
    private void Update()
    {
